...
def genetic_algorithm(pop_size, max_it):
    ...
    for i in range(pop_size):
        P.append(Individual(num_cities, True))

    for i in range(max_it):
        print('ITERATION: ', i)
        new_pop = []
        t = get_threshold(num_cities)

        total_fiteness = 0
        distribution = []
        for individual in P:
            total_fiteness += (t - individual.length)
            distribution.append(total_fiteness)

        for j in range(pop_size):
            X = roulette_wheel(P, distribution)
            Y = roulette_wheel(P, distribution)
            Z = crossover(X, Y)
            new_pop.append(Z)
        P = new_pop

    all_fitness = get_pop_fitness(P)
    fastest_tour = min(all_fitness)
    fastest_index = all_fitness.index(fastest_tour)

    return P[fastest_index].tour
...
def crossover(X, Y):
    ...
    bit_position = random.randint(0, len(X.tour) - 1)

    prefix_X = X.tour[:bit_position]
    suffix_X = X.tour[bit_position:]
    prefix_Y = Y.tour[:bit_position]
    suffix_Y = Y.tour[bit_position:]

    child_1.tour = prefix_X + suffix_Y
    child_2.tour = prefix_Y + suffix_X



    if random.random() < 0.05:
        child_1.tour = mutate(child_1.tour)
    child_1.tour = fix(child_1.tour)

    child_1.length = get_tour_length(child_1.tour)

    if random.random() < 0.05:
        child_2.tour = mutate(child_2.tour)
    child_2.tour = fix(child_2.tour)
...
def fix(tour):
    all_cities = list(range(len(tour)))
    missing_cities = list(set(all_cities) - set(tour))

    seen = set()

    for i in range(len(tour)):
        if tour[i] in seen:
            random_index = random.randrange(len(missing_cities))
            tour[i] = missing_cities.pop(random_index)
        else:
            seen.add(tour[i])
...
def roulette_wheel(P, distribution):

    rand = random.randint(0, distribution[-1] - 1)

    index = bisect.bisect_left(distribution, rand + 1)

    return P[index]

def get_threshold(num_cities):
    # Threshold is calculated by summing the first n largest distances in dist_matrix (where n = num_cities)

    threshold = 0

    flattened = list(itertools.chain(*dist_matrix))
    flattened.sort(reverse=True)

    for i in range(num_cities):
        threshold += flattened[i]
    
    threshold += 1 # Add 1 to ensure threshold is larger than largest distance
...
def get_pop_fitness(P):
    ...
    for i in range(len(P)):
        fitness.append(get_tour_length(P[i].tour))
    return fitness


max_it = 2000
pop_size = 2000
tour = genetic_algorithm(pop_size, max_it)
