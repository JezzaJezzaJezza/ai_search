...
class Particle:
    def __init__(self, num_cities, mu_1, mu_2, mu_3) -> None:
        self.tour = generate_tour(num_cities)
        self.velocity = []

        for _ in range(random.randint(1, num_cities // 4)):
        ...
        self.coef_personal = random.gauss(mu_1, 0.1)
        self.coef_local = random.gauss(mu_2, 0.1)
        self.coef_global = random.gauss(mu_3, 0.1)
        self.forget_ability = 0
...
def PSO(max_it, num_parts):
    ...
    for a in range(num_parts):
        particles.append(Particle(num_cities, mu_1, mu_2, mu_3))
    
    tmp_g_best = min(particles, key=lambda x: x.length)
    g_best = copy.deepcopy(tmp_g_best)

    particles = forget_ability(particles, g_best, p)
    ...
    while t < max_it:
        ...
        for a in range(num_parts):
            ...
            epsilon_prime_prime = random.uniform(0, 1)
            new_tour = []
            
            n = copy.deepcopy(g_best) # Still global best
            new_tour = apply_velocity(particles[a].tour, particles[a].velocity)


            e1 = scalar_mult(particles[a].velocity, theta)
            e2 = scalar_mult(subtract(particles[a].tour, particles[a].current_best), particles[a].coef_personal * epsilon)
            e3 = scalar_mult(scalar_mult(subtract(particles[a].tour, n.tour), (1 - particles[a].forget_ability)), particles[a].coef_local * epsilon_prime)
            e4 = scalar_mult(scalar_mult(subtract(particles[a].tour, g_best.tour.copy()), (1 - particles[a].forget_ability)), particles[a].coef_global * epsilon_prime_prime)

            tmp_velocity = add_velocities(e1, e2)
            tmp_velocity = add_velocities(tmp_velocity, e3)

            particles[a].velocity = add_velocities(tmp_velocity, e4)
            particles[a].tour = new_tour
            particles[a].length = get_tour_length(new_tour)

            if particles[a].length < particles[a].current_best_length:
                particles[a].current_best_length = particles[a].length
                particles[a].current_best = particles[a].tour
        
        pop_best = min(particles, key=lambda x: x.length)

        if pop_best.length < g_best.length:
            g_best.tour = pop_best.tour
            g_best.length = pop_best.length
            stag_global = 0
            print(f'NEW GLOBAL BEST: {g_best.length}')
        else:
            stag_global += 1

        if stag_global >= stag_max:
            particles = forget_ability(particles, g_best, p)
            particles = update_accelerations(particles, eta, p)
...
def generate_tour(num_cities):
    tour = random.sample(range(0, num_cities), (num_cities // 10)) # Generate 10% of a tour randomely
    seen = set(tour)

    tmp_next = 999999999

    for i in range(num_cities - len(tour)):
        for j in range(num_cities):
            if dist_matrix[tour[-1]][j] < tmp_next and j not in seen:
                tmp_next = dist_matrix[tour[-1]][j]
                next_city = j
...
def update_accelerations(particles, eta, p):
    # Assuming that particles is sorted
    elite_counter = math.floor(len(particles) * p)
    avg_mu_1 = sum([particle.mu_1 for particle in particles[:elite_counter]]) / len(particles[:elite_counter])
    avg_mu_2 = sum([particle.mu_2 for particle in particles[:elite_counter]]) / len(particles[:elite_counter])
    avg_mu_3 = sum([particle.mu_3 for particle in particles[:elite_counter]]) / len(particles[:elite_counter])

    for particle in particles:
        particle.mu_1 = (1 - eta) * particle.mu_1 + eta * avg_mu_1
        particle.coef_personal = random.gauss(particle.mu_1, 0.1)
        particle.mu_2 = (1 - eta) * particle.mu_2 + eta * avg_mu_2
        particle.coef_local = random.gauss(particle.mu_2, 0.1)
        particle.mu_3 = (1 - eta) * particle.mu_3 + eta * avg_mu_3
        particle.coef_global = random.gauss(particle.mu_3, 0.1)
    
    return particles

def forget_ability(particles, g_best, p):
    distances = []
    forget_start = math.floor(len(particles) * p)

    for particle in particles:
        distances.append(len(subtract(particle.tour, g_best.tour)))
    
    paired_list = list(zip(distances, particles)) # Sort the particles by their distance to the global best
    paired_list.sort(key = lambda x: x[0])
    distances, particles = zip(*paired_list)
    distances = list(distances)
    particles = list(particles)

    g_worst = get_worst(particles)

    for i in range(forget_start - 1):
        particles[i].forget_ability = 0

    for i in range(forget_start, len(particles) - 1):
        particles[i].forget_ability = (math.e ** (i / len(particles))) * (g_worst.length - g_best.length) * 0.05

    return particles

def get_worst(particles):
    current = particles[0]
    for particle in particles:
        if particle.length > current.length:
            current = particle
    return current
...
def min_tour(new_tour, current_tour):
    if new_tour.length < current_tour.length:
        return new_tour
    else:
        return current_tour
...
def get_best(particles):
    ...
    for particle in particles:
        ...
        if g_best_len == -1: # Checks for optimal tour
            g_best_len = counter
            g_best = particle.tour
        elif counter < g_best_len:
            g_best_len = counter
            g_best = particle.tour
...
def subtract(t1, t2):
    ...
    t2_dict = {t2[i]: i for i in range(len(t2))}

    for i in range(len(tmp_t1) - 1):
        swap = (i, t2_dict[tmp_t1[i]])
...
tour_out = PSO(max_it, num_parts)
tour = tour_out.tour
tour_length = tour_out.length
