...
#Genetic algorithm gives higher quality solutions, but particle swarm gives solutions faster, though they might not be as high quality
...
class Particle:
    def __init__(self, num_cities) -> None:
        self.tour = random.sample(range(0, num_cities), num_cities)
        self.velocity = []

        atomic_ammendmants = random.randint(1, num_cities - 1)
        start_points = sorted(random.sample(range(num_cities - 1), atomic_ammendmants))
        
        for start in start_points:
            swap = (start, start + 1) 
            self.velocity.append(swap)
        self.current_best = self.tour
        self.length = get_tour_length(self.tour)
        self.current_best_length = self.length



    def __repr__(self):
        return f"Tour: {self.tour}, Velocity: {self.velocity}"


def PSO(max_it, num_parts):
    ...
    while t < max_it:
        ...
        for a in range(num_parts):
            ...
            epsilon_prime = random.uniform(0, 1)
            new_tour = []
            n = p_best # Best in some given neighbourhood. In this case take neighbourhood size as infinite
            new_tour = apply_velocity(particles[a].tour, particles[a].velocity)


            e1 = scalar_mult(particles[a].velocity, theta)
            e2 = scalar_mult(subtract(particles[a].tour, particles[a].current_best), alpha * epsilon)
            e3 = scalar_mult(subtract(particles[a].tour, n), beta * epsilon_prime)
            tmp_velocity = add_velocities(e1, e2)

            particles[a].velocity = add_velocities(tmp_velocity, e3)
            particles[a].tour = new_tour

            particles[a].current_best = min_tour(new_tour, particles[a].tour)

        old_best = p_best.copy()
        p_best = min_tour(get_best(particles), p_best)
        if p_best != old_best:
            print(f'NEW BEST: {p_best}')

        t += 1

    return p_best

def scalar_mult(velocity, scalar):
    scalar_int_part = math.floor(scalar)
    scalar_dec_part = scalar - scalar_int_part
    new_velocity = []


    if scalar_int_part != 0:
        new_velocity = scalar_int_part * velocity

    if scalar_dec_part != 0:
        element_counter = math.floor(len(velocity) * scalar_dec_part)
        new_velocity += velocity[:element_counter]
...
def add_velocities(v1, v2):
    arb_tour = list(range(num_cities))
    new_velocity = v1 + v2
    new_velocity = simplify(arb_tour, new_velocity)
    return new_velocity

def simplify(arb_tour, new_velocity):
    tmp_tour = apply_velocity(arb_tour, new_velocity)
    return subtract(arb_tour, tmp_tour)

def apply_velocity(tour, velocity):
    tmp_tour = tour[:]
    for swap in velocity:
        temp = tmp_tour[swap[0]]
        tmp_tour[swap[0]] = tmp_tour[swap[1]]
        tmp_tour[swap[1]] = temp
    return tmp_tour
...
def min_tour(new_tour, current_tour):
    if get_tour_length(new_tour) < get_tour_length(current_tour):
        return new_tour
    else:
        return current_tour
...
def get_best(particles):
    ...
    for particle in particles:
        if current.length < particle.length:
            current = particle

    for particle in particles:
        counter = particle.length
        if p_best_len == -1: # Checks for optimal tour
            p_best_len = counter
            p_best = particle.tour
        elif counter < p_best_len:
            p_best_len = counter
            p_best = particle.tour
...
def subtract(t1, t2):
    ...
    t2_dict = {n:i for i, n in enumerate(t2)}

    for i in range(len(tmp_t1) - 1):
        swap_made = False
        for j in range(0, len(tmp_t1) - i - 1):
            if t2_dict[tmp_t1[j]] > t2_dict[tmp_t1[j + 1]]:
                swaps_made.append((j, j + 1))
                tmp_t1[j], tmp_t1[j + 1] = tmp_t1[j + 1], tmp_t1[j]
                swap_made = True
